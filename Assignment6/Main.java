...
class MyTeacher implements ITeacher {
	...
	public int compare(double[] beliefs, Plan planA, Plan planB, TransitionModel transitionModel, ObservationModel observationModel) {
		...
		//I want to prefer plans that are near .5 because I believe 
		//that is how youp ast most walls?
		if(.5-endObsA[0] < .5-endObsB[0])
			return 1;
		else if(.5-endObsB[0] < .5-endObsA[0])
			return -1;
		else
			return 0;
	}
...
class Main {
	...
	public static void testMarshaling() throws Exception {
		// Make an agent
		final ManicAgent agent = new ManicAgent(new Random(1234), new MyTeacher(), 7, 3, 1);

		final double[] observations = new double[7];
		final double threshold = .935;
		...
		for(int i =0; i<iterations; i++)
		{
			...
			{
				System.out.print((float)((float)i/iterations)*100);
				System.out.print("%\n");
			}
			game.move(); //Moving the game
			observations[0] = game.birdy.y;
			observations[1] = game.wall.y;
			observations[2] = game.wall.x;
			observations[3] = game.wall2.y;
			observations[4] = game.wall2.x;
			if(game.dead)
				observations[5] = -1;
			else
				observations[5] = 1;
			observations[6] = game.score;

			double[] results = agent.think(observations);
		...
		}catch(Exception ex){} //meh, doesn't matter 
