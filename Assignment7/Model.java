...
class Model {
	...
	float getTravelSpeed(float x, float y) {
			...
			int pos = 4 * (60 * yy + xx);
			return Math.max(0.05f, Math.min(3.5f, -0.01f * (terrain[pos + 1] & 0xff) + 0.02f * (terrain[pos + 3] & 0xff)));
	}
	...
	void setName(int sprite, String name)
	{
		sprites_self.get(sprite).setName(name);
	}
